...
void exercise_file_system(FileSystem * _file_system) {
    ...
    file1->Reset();
    char result1[30];
    unsigned int len = file1->Read(40, result1);
    Console::puts("Length is: "); Console::puti(len); Console::puts("\n");
    //assert(file1->Read(40, result1) == 20);
    for(int i = 0; i < 20; i++) {
...
void exercise_file_system(FileSystem * _file_system) {
    ...
    const char * STRING1 = "";
    const char * STRING2 = "abcdefghijabcdefghij";
    const char * STRING3 = "ajphpafhpahfpafjapfjalfjahfpaofahfoa1213";
    ...
    File * file3 = _file_system->LookupFile(3);
    assert(file3 != NULL);
	...
	//Console::puts("k: "); Console::puti(k); Console::puts("\n");
    	file1->Write(1461, STRING1);
}
    Console::puts("File size: ");Console::puti(file1->file_size);Console::puts("\n");
    Console::puts("inode_no: ");Console::puti(file1->inode_no);Console::puts("\n");
    Console::puts("inode_size: ");Console::puti(file1->inode_size);Console::puts("\n");
    Console::puts("cur_position: ");Console::puti(file1->cur_position);Console::puts("\n");
    Console::puts("cur_size: ");Console::puti(file1->cur_size);Console::puts("\n");

    file2->Rewrite();
    file2->Write(20, STRING2);

    file3->Rewrite();
    file3->Write(40, STRING3);
    ...
    file1 = _file_system->LookupFile(1);
    file2 = _file_system->LookupFile(2);
    file3 = _file_system->LookupFile(3);

    file1->Reset();
    char result1[1510];
    for (int k = 0; k < test_time; k++) {
	assert(file1->Read(1461, result1) == 1461);
	for(int i = 0; i < 1461; i++) {
	if (result1[i] != STRING1[i]) {
    ...
    file3->Reset();
    char result3[41];
    assert(file3->Read(80, result3) == 40);
    for(int i = 0; i < 40; i++) {
...
void fun3() {
    ...
    //Console::puts("File size: "); Console::puti(FILE_SYSTEM->file_size); Console::puts("\n");
    for(int j = 0;; j++) {
...
int main() {
    ...
    thread1 = new Thread(fun1, stack1, size);
    Console::puts("DONE\n");

    Console::puts("CREATING THREAD 2...");
    char * stack2 = new char[size];
    thread2 = new Thread(fun2, stack2, size);
    Console::puts("DONE\n");

    Console::puts("CREATING THREAD 3...");
    char * stack3 = new char[size];
    thread3 = new Thread(fun3, stack3, size);
    Console::puts("DONE\n");

    Console::puts("CREATING THREAD 4...");
    char * stack4 = new char[size];
    thread4 = new Thread(fun4, stack4, size);
    Console::puts("DONE\n");
